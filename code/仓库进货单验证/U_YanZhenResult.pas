...
begin
  ...
  temp := temp + ')';
  exsql:='update MatlInDepotZ set state=''YY'' where MIDDONO in '+temp;
  if EXSQLClientDataSet(ClientDataSet1,exsql) then
  begin
    zsbSimpleMSNPopUpShow('操作成功.');
  end
  else
  begin
    ZsbMsgErrorInfoNoApp(F_CangKuJinHuoDanYanZhen,'操作失败,请联系管理员.');
  end;
end;
