...
begin
  ...
  begin
    temp := temp + ' and LZm_ML_MO_OutLog.MatlVer like ''%' + LabeledEdit4.Text + '%''';
  end;
  if LabeledEdit5.Text <> '' then
  begin
    temp := temp + ' and LZm_ML_MO_OutLog.AutoLot like ''%' + LabeledEdit5.Text + '%''';
  end;  
  if LabeledEdit6.Text <> '' then
  begin
    temp := temp + ' and LZm_ML_MO_OutLog.ML_NO like ''%' + LabeledEdit6.Text + '%''';
  end;
  if LabeledEdit7.Text <> '' then
  begin
    temp := temp + ' and LZm_ML_MO_OutLog.ML_NO in (select ML_NO from ZT_ML_NO_FromERP where MO_NO like ''%' + LabeledEdit7.Text + '%'')';
  end;
...
begin
  try
    SaveDialog1.FileName := '后工程发料明细表 ' + FormatDateTime('yyyyMMddHHmmss', Now) + '.xls';
...
begin
  strsql := 'select LZm_ML_MO_OutLog.matlcode,sum(LZm_ML_MO_OutLog.matlquat) matlquat from LZm_ML_MO_OutLog';
  ...
  if LabeledEdit4.Text <> '' then
  begin
    strsql := strsql + ' and MatlVer like ''%' + LabeledEdit4.Text + '%''';
  end;
  if LabeledEdit5.Text <> '' then
  begin
  ...
  if LabeledEdit6.Text <> '' then
  begin
    strsql := strsql + ' and ML_NO like ''%' + LabeledEdit6.Text + '%''';
  end;
  if LabeledEdit7.Text <> '' then
  begin
    strsql := strsql + ' and ML_NO in (select ML_NO from ZT_ML_NO_FromERP where MO_NO like ''%' + LabeledEdit7.Text + '%'')';
  end;
